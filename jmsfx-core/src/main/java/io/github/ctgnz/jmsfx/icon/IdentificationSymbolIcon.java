...
public class IdentificationSymbolIcon extends Pane {
    ...
    protected IdentificationSymbolIcon(IdentificationSymbol symbol) {
        ...
        symbol.codeProperty()
            .addListener((obs, oldValue, newValue) -> updateIcon());
        symbol.amplifierGuidesVisibleProperty()
            .addListener((obs, oldValue, newValue) -> updateIcon());
        symbol.getTextAmplifiers()
            .addListener((MapChangeListener<Amplifier, TextAmplifierValue>) change -> {
                TextAmplifierValue newAmplifier = change.getValueAdded();
                if (newAmplifier != null) {
                    newAmplifier.locationProperty()
                        .addListener((obs, oldValue, newValue) -> updateIcon());
                }
                updateIcon();
            });
        symbol.getGraphicAmplifiers()
            .addListener((MapChangeListener<Amplifier, GraphicAmplifierValue>) change -> {
                GraphicAmplifierValue newGraphic = change.getValueAdded();
                if (newGraphic != null) {
                    newGraphic.transformProperty()
                        .addListener((obs, oldValue, newValue) -> updateIcon());
                }
                updateIcon();
            });
    ...
    protected void updateIcon() {
        ...
        if (symbol.isStatusIconUsed()) {
            container.getChildren()
                .add(symbol.getStatusGraphic()
                    .createGroup());
        }
        if (symbol.isHqtfDummyIconUsed()) {
            container.getChildren()
                .add(symbol.getHqtfDummyGraphic()
                    .createGroup());
        }
        if (symbol.isMainIconUsed()) {
            container.getChildren()
                .add(symbol.getMainIconGraphic()
                    .createGroup());
        }
        if (symbol.isAmplifierUsed()) {
            container.getChildren()
                .add(symbol.getAmplifierGraphic()
                    .createGroup());
        }
        if (symbol.isAmplifierTwoUsed()) {
            container.getChildren()
                .add(symbol.getAmplifierTwoGraphic()
                    .createGroup());
        }
        if (symbol.isAmplifierThreeUsed()) {
            container.getChildren()
                .add(symbol.getAmplifierThreeGraphic()
                    .createGroup());
        }
        if (symbol.isSectorOneModifierUsed()) {
            container.getChildren()
                .add(symbol.getSectorOneModifierGraphic()
                    .createGroup());
        }
        if (symbol.isSectorTwoModifierUsed()) {
            container.getChildren()
                .add(symbol.getSectorTwoModifierGraphic()
                    .createGroup());
        }
        ...
        symbol.getTextAmplifiers()
            ...
            .forEach(textAmplifier -> {
                ...
                text.textProperty()
                    .bind(textAmplifier.textProperty());
                container.getChildren()
                    .add(text);
            });
        symbol.getGraphicAmplifiers()
            .values()
            .forEach(graphicAmplifier -> {
                Group group = graphicAmplifier.getGraphic()
                    .createGroup();
                group.getTransforms()
                    .add(graphicAmplifier.getTransform());
                container.getChildren()
                    .add(group);
            });
        Bounds lb = container.getLayoutBounds();
        double scaleFactor = symbol.getScale()
            .getFactor();
